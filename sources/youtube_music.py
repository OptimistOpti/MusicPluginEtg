...
_external_log = [None]  # устанавливается из плагина
...
def _log(msg):
    ...
    if fn:
        ...
        _log_internal.info(str(msg))

_log_internal = logging.getLogger("music")
...
def _find_working_instance(path, timeout=4.0):
    ...
    if result[0]:
        ...
        _log(f"[music] working instance: {inst}")
        return inst, resp
...
def get_stream_url(video_id: str, instance: str = None) -> dict:
    ...
    for inst in priority:
        try:
            ...
            _log(f"[music] stream error {inst}: {e}")

    if not data:
        inst, resp = _find_working_instance(path, timeout=8)
        if not inst:
            _log("[music] all instances failed")
            return {}
        data = resp.json()
        used_inst = inst
...
def _pick_best_stream(data: dict, inst: str) -> dict:
    ...
    if not audio_streams:
        _log(f"[music] no audioStreams, keys: {list(data.keys())}")
        return {}

    _log(f"[music] {len(audio_streams)} audio streams from {inst}:")
    for s in audio_streams:
        _log(f"  format={s.get('format')} bitrate={s.get('bitrate')} mime={s.get('mimeType')} url={s.get('url','')[:60]}")
    ...
    _log(f"[music] selected: format={best.get('format')} bitrate={best.get('bitrate')} mime={mime}")
