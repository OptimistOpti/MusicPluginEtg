...
def get_stream_url(video_id: str, instance: str = None) -> str:
    """
    Возвращает proxyUrl аудиопотока — трафик идёт через Piped прокси,
    обходя блокировку YouTube по IP (*.googlevideo.com недоступны напрямую).
    """
    print(f"[music] get_stream_url: {video_id}")
    path = f"/streams/{video_id}"

    # Приоритет: переданный инстанс → кешированный → параллельный перебор
    priority = []
    if instance:
    ...
    for inst in priority:
        try:
            ...
            resp.raise_for_status()
            data = resp.json()
            url = _extract_best_stream(data, inst)
            if url:
                return url
    ...
    if not inst:
        print(f"[music] all instances failed for {video_id}")
        return ""

    return _extract_best_stream(resp.json(), inst) or ""


def _extract_best_stream(data: dict, inst: str) -> str:
    audio_streams = data.get("audioStreams", [])
    if not audio_streams:
        print(f"[music] no audioStreams, keys: {list(data.keys())}")
        return ""

    best = max(audio_streams, key=lambda s: s.get("bitrate", 0))

    # proxyUrl пускает трафик через Piped сервер — обходит блокировку YouTube
    url = best.get("proxyUrl") or best.get("url", "")
    print(f"[music] stream from {inst}, bitrate={best.get('bitrate')}, proxyUrl={'yes' if best.get('proxyUrl') else 'no'}")
    return url
