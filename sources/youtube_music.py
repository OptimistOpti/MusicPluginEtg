...
def get_stream_url(video_id: str, instance: str = None) -> dict:
    ...
    if not data:
        ...
        if not inst:
            print("[music] all instances failed")
            return {}
        data = resp.json()
        used_inst = inst
...
def _pick_best_stream(data: dict, inst: str) -> dict:
    ...
    print(f"[music] {len(audio_streams)} audio streams from {inst}:")
    for s in audio_streams:
        print(f"  format={s.get('format')} bitrate={s.get('bitrate')} mime={s.get('mimeType')} url={s.get('url','')[:60]}")
    ...
    print(f"[music] selected: format={best.get('format')} bitrate={best.get('bitrate')} mime={mime}")
